...
object Exercises extends App {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        var mutable_number = number
        var result: Seq[Int] = List()

        while (mutable_number % 2 == 0) {
            result :+= 2
            mutable_number = mutable_number / 2
        }

        for (i <- 3 to Math.sqrt(mutable_number).toInt by 2) {
            while (mutable_number % i == 0) {
                result :+= i
                mutable_number = mutable_number / i
            }
        }
    ...
    def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D, scalarOrCos: (Vector2D, Vector2D) => Double, rightVec0: Vector2D, rightVec1: Vector2D) = {
        scalarOrCos(leftVec0, leftVec1) + scalarOrCos(rightVec0, rightVec1)
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        var weightMap = ballsArray.map{ case (name, properties) =>
            name -> 4/3 * Math.PI * Math.pow(properties._1, 3) * properties._2
        }
        ListMap(weightMap.toSeq.sortWith(_._2 < _._2):_*).keys.toSeq
    }
